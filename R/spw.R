...
#' @param correction In case that p-value correction for multiple tests is required, use this parameter to specify which correction must be used. See ?p.adjust for details of available correction methods. It is possible to have more than one correction in one go, using a vector with correction names in the \code{p.adjust} function fashion (e.g. c("bonferroni", "fdr", "hochberg")). Each correction will add a column to the output.
